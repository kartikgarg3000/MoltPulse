...
export default function AdminDashboard() {
  ...
  const handleApprove = async (sub: any) => {
    ...
    try {
      ...
      alert(`✅ Agent "${result.agent.name}" added to directory!\n\nPulse Score: ${result.agent.pulse_score}\nQuality Score: ${result.agent.quality_score}\nCategory: ${result.agent.category}`);
      fetchSubmissions();
    } catch (err: any) {
                    ...
                    {sub.status === 'pending' && (
                      <>
                        <button 
                          ...
                          disabled={ingesting === sub.id}
                          className="flex-1 py-3 bg-green-600 hover:bg-green-500 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          {ingesting === sub.id ? (
                            <><RefreshCw size={16} className="animate-spin" /> Ingesting...</>
                          ) : (
                            <><Check size={16} /> Approve & Ingest</>
                          )}
                        </button>
                        <button 
                          onClick={() => handleReject(sub.id)}
                          disabled={ingesting === sub.id}
                          className="flex-1 py-3 bg-red-600/20 hover:bg-red-600 text-red-400 hover:text-white rounded-xl font-bold text-sm transition-all border border-red-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          <X size={16} /> Reject
                        </button>
                      </>
